...
class Spec(object):
    def __init__(self):
        ...
        self.counts = []
        self.energy = []
        self.en_coef = [0, 0, 0, 0]
        self.unit = ''
        self.fwhm = []
        self.n_ch = 0
        self.real = 0
        self.live = 0
        self.fw_coef = [0, 0, 0, 0]
        self.start = 0
        self.tot = 0     
        self.name = ''
        self.mode = ''
        ...
        self.bkg = None
        self.maxc = None
        self.eff = None   # {'coef': [0, 0, 0, 0], 'data': [ [energias],[energias],[errores] ]} 
        # self.exist = False


def read_cnf_file(filename,write_output=False, d_o=False):
    """
    ...
    if set(('Energy','Shape coefficients')) <= set(read_dic):
        read_dic.update(shape_from_energy(read_dic))
    ...
    if d_o:
        ...
        spec = Spec()
        spec.channels = read_dic['Channels']
        spec.counts = read_dic['Channels data']
        spec.energy = read_dic['Energy']
        spec.en_coef = read_dic['Energy coefficients']
        spec.unit = read_dic['Energy unit']
        spec.fwhm = read_dic['FWHM']
        spec.n_ch = read_dic['Number of channels']
        spec.real = read_dic['Real time']
        spec.live = read_dic['Live time']
        spec.fw_coef = read_dic['Shape coefficients']
        spec.start = read_dic['Start time']
        spec.tot = read_dic['Total counts']
        spec.maxc = read_dic['Channels data'].max()
        spec.name = read_dic['MCA type']
        spec.mode = read_dic['Measurement mode']
        # spec.exist = True
...
def get_energy_calibration(f,offs_param):
    ...
    A = np.empty(4)
    A[3] = pdp11f_at(f, offs_calib + 0x44)
    A[2] = pdp11f_at(f, offs_calib + 0x48)
    A[1] = pdp11f_at(f, offs_calib + 0x4c)
    A[0] = pdp11f_at(f, offs_calib + 0x50)
...
"""Added Shape Calibration Parameters : FWHM=B[1]+B[0]*E^(1/2)  . B[3] and B[2]
...
def get_shape_calibration(f,offs_param):
    ...
    offs_calib = offs_param + 0x30 + uint16_at(f, offs_param + 0x22)
    B = np.empty(4)
    B[1] = pdp11f_at(f, offs_calib + 0xdc)
    B[0] = pdp11f_at(f, offs_calib + 0xe0)
    B[3] = pdp11f_at(f, offs_calib + 0xe4)
    B[2] = pdp11f_at(f, offs_calib + 0xe8)
...
def get_date_time(f,offs_param):
    ...
    start_time_str = np.datetime64(time.strftime('%Y-%m-%dT%H:%M:%S',time.gmtime(start_time)))
...
def chan_to_energy(dic):
    ...
    ch= dic['Channels']
    energy = A[3] + A[2]*ch + A[1]*ch*ch + A[0]*ch*ch*ch
    out_dic = {'Energy': energy}

    return  out_dic

def shape_from_energy(dic):
    """ Calculate the FWHM in energy units, using shape calibration coefficients."""

    B = dic['Shape coefficients']
    E = cp.copy(dic['Energy'])
    E[E<0]=0
    fwhm = B[1] + B[0]*np.sqrt(E)
...
def write_to_file(filename,dic):
    ...
    with open(filename,'w') as f: 
        ...
        f.write('#   n   Energy({})     FWHM({})    Counts\n'.format(dic['Energy unit'],dic['Energy unit']))
        f.write('#'+50*'-'+'\n')
        for i,j,k,m in zip(dic['Channels'],dic['Energy'],dic['FWHM'],dic['Channels data']):
            f.write('#{:4d}\t{:.3e}\t{:.3e}\t{}\n'.format(i,j,k,m))
...
if __name__ == "__main__":
    ...
    # else:
    #     filename = sys.argv[1]
    filename='files/eu3.cnf'
    
    c = read_cnf_file(filename, write_output=False, d_o=True) 
